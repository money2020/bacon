(function() {
  angular.module('money2020.bacon.router', []).constant("ROUTES", {
    '/': {
      templateUrl: '/partials/partial-transactions.html',
      controller: 'TransactionsController',
      resolve: {
        State: function(TransactionAPI, AppState) {
    ...
    var options, path, _results;
    _results = [];
    for (path in ROUTES) {
      options = ROUTES[path];
      _results.push($routeProvider.when(path, options));
    }
    return _results;
  });
